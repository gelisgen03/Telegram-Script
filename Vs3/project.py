...
with client:
    ...
    messages = client.get_messages(secilen_kullanici, limit=1)  # Son limit=... mesajı al
    database = db.cursor()
    ...
    for message in messages:
        try:
            ...
            #veritabanına eklenen mesajın id değerini almak için lastrowid kullanılabilir. 
            #Bu, son eklenen satırın birincil anahtarını döndürür.
            message_id = database.lastrowid  # Son eklenen mesajın ID'sini al

            # 4. AI ile mesajı işleyip özet oluştur
            if msg_text:  # Sadece metin içeren mesajlar için özet oluştur
                msg_summary = gemini_ai.get_summary(msg_text)  # AI'dan özet al
                print(f"AI Özeti: {msg_summary}")

                # 5. Özeti summary tablosuna kaydet
                database.execute("""
                    INSERT INTO summary (msg_id, msg_sum)
                    VALUES (%s, %s)
                """, (message_id, msg_summary))
                db.commit()
                print("Özet başarıyla summary tablosuna kaydedildi.")
            else:
                print("Bu mesajda metin bulunmuyor, özet oluşturulmadı.")
